...
for f in glob.glob("./*.xlsx"):
    year, month = int(f.replace('By_Airline_', '')[2:6]), int(f.replace('By_Airline_', '')[6:8])
    df = pd.read_excel(f)
    # 엑셀의 첫 행이 의미 없음
    ...
    condition_df.insert(0, 'year', [year] * condition_df.shape[0])
    condition_df.insert(1, 'month', [month] * condition_df.shape[0])
...
all_data.to_csv('./passengers.csv')
```
...
0,2017,1,대한항공,756322
1,2017,1,델타항공,16199
2,2017,1,루프트한자 독일항공,11509
3,2017,1,아메리칸항공,7943
4,2017,1,아시아나항공,541993
5,2017,1,에미레이트항공,14695
6,2017,1,에어 프랑스,9143
7,2017,1,에어서울,31457
8,2017,1,유나이티드항공,14834
9,2017,1,제주항공,178981
...
```
