...
def predict(airline):
    ...
    if MAPE(df_test, forecast_3) < MAPE(df_test, forecast_5):
        Holt(df, exponential=True).fit().forecast(3).astype(int).reset_index().rename(columns={'index':'date', 0:'passengers'}).to_csv('./predict_data/%s.csv' % convert_dict[airline])
    else:
        Holt(df, exponential=True, damped_trend=True).fit().forecast(3).astype(int).reset_index().rename(columns={'index':'date', 0:'passengers'}).to_csv('./predict_data/%s.csv' % convert_dict[airline])
